              ...
              <template v-for="round in rounds">
                <tr :key="round.id">
                  ...
                  <td class="project-actions text-right">
                    ...
                    <font-awesome-icon :icon="['fa', opened.includes(round.id) ? 'minus' : 'plus']" @click="toggle(round.id)" :class="{ opened: opened.includes(round.id)}" />
                  </td>
                </tr>
                <tr style="padding-left: 20px" v-if="opened.includes(round.id) && round.bets && round.bets.length" :key="`i-${round.id}`">
<table>
            <thead>
              <tr>
                <th style="width: 1%">
